...
import copy
import math
from experiment3.NRMSE import rmse, rmse2
import os
...
class DANSE(nn.Module):
    ...
    def __init__(self, n_states, n_obs, mu_w, C_w, H, mu_x0, C_x0, batch_size, rnn_type, rnn_params_dict, device='cpu'):
        ...
        self.H = self.push_to_device(H)         #直接加张量
    ...
    def compute_logpdf_Gaussian(self, Y):
        ...
        logprob = 0.5 * self.n_obs * T * math.log(math.pi*2) - 0.5 * torch.logdet(self.L_yt_current).sum(1) \
            - 0.5 * torch.einsum('nti,nti->nt', (Y - self.mu_yt_current),
...
def train_danse(model, options, train_loader, val_loader, nepochs, logfile_path, modelfile_path, save_chkpoints, pri_num, device='cpu', tr_verbose=False):
    ...
    try:
        for epoch in range(nepochs+1):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                ######################################
                optimizer.zero_grad()
                Y_train_batch = Variable(tr_Y_batch, requires_grad=False).type(torch.DoubleTensor).to(device)
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(val_loader, 0):
                    ...
                    Y_val_batch = Variable(val_Y_batch, requires_grad=False).type(torch.DoubleTensor).to(device)
                    ...
                    val_mse_loss_batch = rmse(val_X_batch, val_mu_X_filtered_batch)
                    # print statistics
                ...
                val_mse_loss = 10*torch.log10(val_mse_loss_batch)
                ...
                if tr_verbose == True and (((epoch + 1) % pri_num) == 0 or epoch == 0):

                    print("Epoch: {}/{}, Training NLL:{:.9f}, Val. NLL:{:.9f}, Val. MSE:{:.4f}dB".format(epoch+1,
                    model.rnn.num_epochs, tr_loss, val_loss, val_mse_loss), file=orig_stdout)

                    danse_loss += [val_mse_loss]
                    #save_model(model, model_filepath + "/" + "{}_ckpt_epoch_{}.pt".format(model.model_type, epoch+1))

                    # print("Epoch: {}/{}, Training NLL:{:.9f}, Val. NLL:{:.9f}, Val. MSE: {:.4f}dB, Time_Elapsed:{:.4f} secs".format(epoch+1,
            ...
            if val_loss < loss_best:
                ...
                best_val_epoch = epoch+1               # Corresponding value of epoch
                best_model_wts = copy.deepcopy(model.state_dict()) # Weights for the best model
                # save_model(model, model_filepath + "/" + "{}_usenorm_best_ckpt_epoch_{}.pt".format(model.rnn_type, epoch+1))
    ...
    return tr_losses, val_mse_loss, danse_loss, tr_loss_for_best_val_loss, model
...
def test_danse(test_loader, options, device, model_file=None, test_logfile_path = None):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader, 0):
            ...
            Y_test_batch = Variable(te_Y_batch, requires_grad=False).type(torch.DoubleTensor).to(device)
            ...
            est_mean = te_mu_X_filtered_batch.squeeze(0)
            est_var = te_var_X_filtered_batch.squeeze(0)
            test_mse_loss_batch2 = rmse(te_X_batch[:,:,:], te_mu_X_filtered_batch[:,:,:])
            test_mse_loss_batch = rmse2(te_X_batch[0, 1:, :], te_mu_X_filtered_batch[0, 1:, :])
            # print statistics
    ...
    test_mse_loss = 10*torch.log10(test_mse_loss_batch)
    test_mse_loss2 = 10 * torch.log10(test_mse_loss_batch2)
    test_NLL_loss = te_log_pY_epoch_sum

    # print('Test NLL loss: {:.3f}, Test MSE loss: {:.4f}dB using weights from file: {} %'.format(test_NLL_loss, test_mse_loss, model_file))

    # with open(test_log, "a") as logfile_test:
    #     logfile_test.write('Test NLL loss: {:.3f}, Test MSE loss: {:.4f}dB using weights from file: {}'.format(test_NLL_loss, test_mse_loss, model_file))
    ...
    est_var = est_var.diagonal(dim1=-2, dim2=-1)
    return test_mse_loss, test_mse_loss2, est_mean, est_var
