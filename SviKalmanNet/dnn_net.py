...
class ConvGate(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(1, 2, 0)      # (batch, hidden_dim, seq_len)
        gate = self.conv(x)         # 卷积操作
        gate = self.sigmoid(gate)
        gate = gate.permute(2, 0, 1)  # (seq_len, batch, hidden_dim)
...
class DNN_SKalmanNet(torch.nn.Module):
    ...
    def forward(self, state_inno, precov, residual, meas_cov):
        ...
        X_hat = self.l2(attention_out)
        var_out = self.l3(attention_out)
        P_out = var_out ** 2

        x_hat = X_hat.reshape(self.x_dim, 1)
        P_hat = P_out.reshape(self.x_dim, 1)
